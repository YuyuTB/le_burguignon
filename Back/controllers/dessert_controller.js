const Item = require('../models/dessert');
const fs = require('fs');
const {
...
const itemController = {
	...
	getItemById: async (req, res) => {
		const Dessert_id = req.params.Dessert_id;

		try {
			const item = await Item.findByPk(Dessert_id);
	...
	updateItem: async (req, res) => {
		const Dessert_id = req.params.Dessert_id;
		try {
			const existingItem = await Item.findByPk(Dessert_id);
			await uploadUpdate(req, res, existingItem, Item, 'Dessert_id');
		} catch (error) {
			console.error(error);
			res.status(500).send(
				`Erreur lors de la mise à jour de l'item avec l'ID ${Dessert_id}.`
			);
		}
	},
	...
	deleteItem: async (req, res) => {
		...
		try {
			...
			console.error(error);
			res.status(500).send(
				`Erreur lors de la suppression de l'item avec l'ID ${Dessert_id}.`
			);
		}
	},
};
