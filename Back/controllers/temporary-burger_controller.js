const Item = require('../models/temporary-burger');
const fs = require('fs');
const {
...
const itemController = {
	...
	getItemById: async (req, res) => {
		const TemporaryBurger_id = req.params.TemporaryBurger_id;

		try {
			const item = await Item.findByPk(TemporaryBurger_id);
	...
	updateItem: async (req, res) => {
		const TemporaryBurger_id = req.params.TemporaryBurger_id;
		try {
			const existingItem = await Item.findByPk(TemporaryBurger_id);
			await uploadUpdate(
				req,
				res,
				existingItem,
			...
			console.error(error);
			res.status(500).send(
				`Erreur lors de la mise à jour de l'item avec l'ID ${TemporaryBurger_id}.`
			);
		}
	},
	...
	deleteItem: async (req, res) => {
		...
		try {
			...
			fs.unlink('images/' + filename, () => {
				Item.destroy({ where: { TemporaryBurger_id } })
					.then(() => {
			...
			console.error(error);
			res.status(500).send(
				`Erreur lors de la suppression de l'item avec l'ID ${TemporaryBurger_id}.`
			);
		}
	},
};
